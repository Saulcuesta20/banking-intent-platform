...
- Store concepts as `ontology_nodes` and data-driven synonym maps as `ontology_aliases` on flow JSON.
...
## Data Flow
Ingestion extracts and validates `ontology_nodes` on each flow. Normalization
loads the synonym catalog from `data/ontology/term_synonyms.json` or
`ONTOLOGY_SYNONYM_CATALOG_PATH`, then builds `ontology_aliases`. Graph loading creates
...
during ingestion or query understanding. Synonym vocabulary is maintained as

...
| Validate JSON | Custom deterministic | `normalize_and_validate` and `_normalize_*` | Rejects bad schema, missing references, backend operations modeled as user tasks, and adds deterministic ontology aliases. |
...
## Data Flow
`raw corpus -> custom scan/parse -> optional AutoGen findings -> LLM schema extraction -> custom validation + ontology alias normalization -> preview/apply JSON -> audit record -> Neo4j graph load`

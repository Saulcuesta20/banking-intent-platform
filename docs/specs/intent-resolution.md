...
The default `make ask` path uses GraphRAG reasoning in `app/retrieval/graph.py` and `app/intent/ai.py`. Runtime flow context is a projection over ingested knowledge; plan and user-task creation live in ingestion. If the LLM, LangGraph, or Neo4j path is unavailable, ask fails with a configuration error instead of choosing a local non-LLM intent.

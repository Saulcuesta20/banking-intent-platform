...
`Question understanding -> Neo4j graph retrieval -> LangChain prompt template -> LLM JSON decision -> answer building`
...
against flow metadata, utterances, concepts, and concept aliases.
...
| 6 | Knowledge graph service | `KnowledgeGraphService.search` | Delegates the understood search terms to the repository. | Candidate `KnowledgeRecord` list. |
| 7 | Graph search | `Neo4jKnowledgeGraphRepository.search` | Calls `_query_graph_context(search_terms)`, maps Neo4j rows to approved flow records, attaches `graph_context` metadata. | Candidate records with graph context. |
| 8 | Neo4j query | `Neo4jKnowledgeGraphRepository._query_graph_context` | Runs filtered Cypher over `Flow`, `Utterance`, `Concept`, `Synonym`, `UserTask`, and `Action` nodes. It matches search terms against flow id/name/intent/event/explanation/utterances/concept/aliases. | Ranked graph rows. |
| 9 | Intent classification | `FlowSelectionService.select` | Delegates to semantic reasoning provider. | Selected record or `None`. |
| 10 | LangChain prompt | `LLMFlowSelectionProvider.select_intent` | Uses `PromptTemplate` to combine question + graph context into a constrained prompt. | Prompt string. |
...
| 12 | Flow validation | `LLMFlowSelectionProvider.select_intent` | Accepts the LLM choice only if `selected_flow_id` matches an existing candidate flow. | Selected `KnowledgeRecord`. |
| 13 | Flow projection | `AnswerBuilder.build` | Reads ingested event, plan, tasks, actions, and concepts from selected flow. | `AnswerContext`. |
...
| 16 | Output | `AnswerResult.to_dict` | Serializes answer for CLI/API. | JSON-like result. |
...
- selected flow, reason, projected event/plan/tasks/actions/concept
...
- graph rows preview
- concept aliases returned by graph rows
...
## Safety Boundary
The LLM can only choose one existing `flow_id` from retrieved graph context. It does not create plans, tasks, actions, events, or concepts during ask question.

...
- Allow LlamaIndex, Neo4j, GraphRAG, LangGraph, and other tools to be swapped.
...
- `app/ingestion/orchestrator.py::IngestionOrchestratorService`
- `app/ingestion/orchestrator.py::IngestionReasoningProvider`
...
Provider modules live inside the component they implement. For example,
capability providers live under `app/capability`, knowledge-base ports and
adapters live under `app/knowledge_base`, and runtime answer building lives
under `app/ask`.
...
`AskState` workflow, `app/knowledge_base/adapters/graph/neo4j.py` owns Neo4j
retrieval, `app/knowledge_base/vocabulary.py` owns deterministic entity
normalization and synonym aliases, and `app/ask/ai.py` owns LangChain prompt
orchestration plus LLM classification. The LLM receives only knowledge-base
candidate flows/tools and must not invent new tasks.

...
class OrchestratorService:
    ...
    def __init__(
        ...
        event_sink: OrchestratorEventSink | None = None,
    ):
        self.repository = repository or InMemoryOrchestratorRepository()
        self.event_sink = event_sink or InMemoryOrchestratorEventSink()
    ...
    def create_pending_jobs(self, process: ProcessDefinition, instance: OrchestratorInstance) -> OrchestratorInstance:
        ...
        for definition in process.jobs:
            ...
            jobs.append(job)
        updated = instance.model_copy(update={"pending_jobs": [*instance.pending_jobs, *jobs], "updated_at": datetime.utcnow()})
    ...
    def correlate_message(
        ...
        updated = instance.model_copy(update={"data": data, "status": "running", "updated_at": datetime.utcnow()})
        self.repository.save_instance(updated)
    ...
    def complete_instance(self, instance_id: str) -> OrchestratorInstance:
        instance = self._require_instance(instance_id)
        updated = instance.model_copy(update={"status": "completed", "updated_at": datetime.utcnow()})
        self.repository.save_instance(updated)

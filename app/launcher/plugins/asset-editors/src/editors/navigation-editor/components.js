...
import definitionSchema from './navigation-editor.schema.json'
import definitionUiSchema from './navigation-editor.ui-schema.json'
import {
...
function NavigationDefinitionJsonFormsView({ value, onChange, readOnly = false }) {
  const payload = clone(value?.payload)
  const definition = useMemo(() => normalizeNavigationDefinition(payload), [payload])
  return h(
    EditorFrame,
    {
      title: 'Navigation Definition',
...
function HistoryPanel({ value, validation }) {
  ...
  return h(
    EditorFrame,
    {
      ...
      subtitle: 'Navigation structure overview.',
    },
    h('div', { style: { color: colors.muted, fontSize: 12, lineHeight: 1.6 } }, `Items: ${items.length}. Validation: ${validation.valid ? 'ok' : 'review needed'}.`),
...
export function NavigationEditorView({ value, onChange, readOnly = false }) {
  const document = useMemo(() => normalizeDocument(value), [value])
  const [tab, setTab] = useState('structured')
  const validation = useMemo(() => validateNavigationDocument(document), [document])
  ...
  return h(
    ...
    h(
      ...
      h('div', { style: { marginLeft: 'auto', display: 'flex', gap: 8, flexWrap: 'wrap' } }, h(Pill, { label: validation.valid ? 'valid' : 'needs review', tone: validation.valid ? colors.greenSoft : colors.redSoft, textColor: validation.valid ? colors.green : colors.red })),
    ),
    tab === 'structured'
      ? h(
          'div',
          { style: { display: 'grid', gap: 14 } },
          h(NavigationDefinitionJsonFormsView, { value: document, onChange: commit, readOnly }),
        )
      : null,

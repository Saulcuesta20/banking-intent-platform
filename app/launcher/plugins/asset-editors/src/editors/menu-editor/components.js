...
import definitionSchema from './menu-editor.schema.json'
import definitionUiSchema from './menu-editor.ui-schema.json'
import {
...
function MenuDefinitionJsonFormsView({ value, onChange, readOnly = false }) {
  const payload = clone(value?.payload)
  const definition = useMemo(() => normalizeMenuDefinition(payload), [payload])
  return h(
    EditorFrame,
    {
...
function HistoryPanel({ value, validation }) {
  ...
  return h(
    EditorFrame,
    {
      ...
      subtitle: 'Menu structure overview.',
    },
    h('div', { style: { color: colors.muted, fontSize: 12, lineHeight: 1.6 } },
      `Menu ID: ${payload.id || '—'}. Path: ${payload.path || '—'}. Validation: ${validation.valid ? 'ok' : 'review needed'}.`,
    ),
...
export function MenuEditorView({ value, onChange, readOnly = false }) {
  const document = useMemo(() => normalizeDocument(value), [value])
  const [tab, setTab] = useState('structured')
  const validation = useMemo(() => validateMenuDocument(document), [document])
  ...
  return h(
    ...
    tab === 'structured'
      ? h(
          'div',
          { style: { display: 'grid', gap: 14 } },
          h(MenuDefinitionJsonFormsView, { value: document, onChange: commit, readOnly }),
        )
      : null,

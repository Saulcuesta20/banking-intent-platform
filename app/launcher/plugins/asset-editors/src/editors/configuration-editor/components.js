...
import definitionSchema from './configuration-editor.schema.json'
import definitionUiSchema from './configuration-editor.ui-schema.json'
import {
...
function ConfigurationDefinitionJsonFormsView({ value, onChange, readOnly = false }) {
  const payload = clone(value?.payload)
  const definition = useMemo(() => normalizeConfigurationDefinition(payload), [payload])
  return h(
    EditorFrame,
    {
...
function HistoryPanel({ value, validation }) {
  ...
  return h(
    EditorFrame,
    {
      ...
      subtitle: 'Configuration structure overview.',
    },
    h('div', { style: { color: colors.muted, fontSize: 12, lineHeight: 1.6 } },
      `Config ID: ${payload.config_id || '—'}. Scope: ${payload.scope || '—'}. Validation: ${validation.valid ? 'ok' : 'review needed'}.`,
    ),
...
export function ConfigurationEditorView({ value, onChange, readOnly = false }) {
  const document = useMemo(() => normalizeDocument(value), [value])
  const [tab, setTab] = useState('structured')
  const validation = useMemo(() => validateConfigurationDocument(document), [document])
  ...
  return h(
    ...
    tab === 'structured'
      ? h(
          'div',
          { style: { display: 'grid', gap: 14 } },
          h(ConfigurationDefinitionJsonFormsView, { value: document, onChange: commit, readOnly }),
        )
      : null,

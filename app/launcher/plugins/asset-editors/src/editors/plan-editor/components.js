...
import definitionSchema from './plan-editor.schema.json'
import definitionUiSchema from './plan-editor.ui-schema.json'
import {
...
function PlanDefinitionJsonFormsView({ value, onChange, readOnly = false }) {
  const payload = clone(value?.payload)
  const definition = useMemo(() => normalizePlanDefinition(payload), [payload])
  return h(
    EditorFrame,
    {
...
function HistoryPanel({ value, validation }) {
  ...
  return h(
    EditorFrame,
    {
      ...
      subtitle: 'Plan structure overview.',
    },
    h('div', { style: { color: colors.muted, fontSize: 12, lineHeight: 1.6 } },
      `Plan ID: ${payload.plan_id || '—'}. Steps: ${payload.steps?.length || 0}. Validation: ${validation.valid ? 'ok' : 'review needed'}.`,
    ),
...
export function PlanEditorView({ value, onChange, readOnly = false }) {
  const document = useMemo(() => normalizeDocument(value), [value])
  const [tab, setTab] = useState('structured')
  const validation = useMemo(() => validatePlanDocument(document), [document])
  ...
  return h(
    ...
    tab === 'structured'
      ? h(
          'div',
          { style: { display: 'grid', gap: 14 } },
          h(PlanDefinitionJsonFormsView, { value: document, onChange: commit, readOnly }),
        )
      : null,

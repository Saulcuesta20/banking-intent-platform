...
import definitionSchema from './document-editor.schema.json'
import definitionUiSchema from './document-editor.ui-schema.json'
import {
...
function normalizeDocumentAsset(value) {
  const document = clone(value)
  const payload = clone(document.payload || {})
  const docDef = normalizeDocumentDefinition(payload)
...
function DocumentDefinitionJsonFormsView({ value, onChange, readOnly = false }) {
  const payload = clone(value?.payload)
  const definition = useMemo(() => normalizeDocumentDefinition(payload), [payload])
  return h(
    EditorFrame,
    {
...
function RelationsPanel({ value }) {
  ...
  return h(
    ...
    {
      title: 'Relations',
      subtitle: 'Associated module and domain links.',
    },
    relations.length === 0
    ...
    h('div', { style: { marginTop: 12, fontSize: 12, color: colors.muted } },
      `Module: ${payload.module_id || '—'}  ·  Domain: ${payload.domain_id || '—'}`,
    ),
  )
}
...
function HistoryPanel({ value, validation }) {
  ...
  return h(
    EditorFrame,
    {
      ...
      subtitle: 'Document structure overview.',
    },
    h('div', { style: { color: colors.muted, fontSize: 12, lineHeight: 1.6 } },
      `Document ID: ${payload.document_id || '—'}. Title: ${payload.title || '—'}. Validation: ${validation.valid ? 'ok' : 'review needed'}.`,
    ),
...
export function DocumentEditorView({ value, onChange, readOnly = false }) {
  const document = useMemo(() => normalizeDocumentAsset(value), [value])
  const [tab, setTab] = useState('structured')
  const validation = useMemo(() => validateDocumentDefinition(document), [document])
  ...
  return h(
    ...
    tab === 'structured'
      ? h(
          'div',
          { style: { display: 'grid', gap: 14 } },
          h(DocumentDefinitionJsonFormsView, { value: document, onChange: commit, readOnly }),
        )
      : null,

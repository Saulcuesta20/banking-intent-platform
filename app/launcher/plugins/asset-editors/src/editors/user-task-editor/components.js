...
import definitionSchema from './user-task-editor.schema.json'
import definitionUiSchema from './user-task-editor.ui-schema.json'
import {
...
function UserTaskDefinitionJsonFormsView({ value, onChange, readOnly = false }) {
  const payload = clone(value?.payload)
  const definition = useMemo(() => normalizeUserTaskDefinition(payload), [payload])
  return h(
    EditorFrame,
    {
...
function HistoryPanel({ value, validation }) {
  ...
  return h(
    EditorFrame,
    {
      ...
      subtitle: 'User task structure overview.',
    },
    h('div', { style: { color: colors.muted, fontSize: 12, lineHeight: 1.6 } },
      `Task ID: ${payload.user_task_id || '—'}. Type: ${payload.type || '—'}. Validation: ${validation.valid ? 'ok' : 'review needed'}.`,
    ),
...
export function UserTaskEditorView({ value, onChange, readOnly = false }) {
  const document = useMemo(() => normalizeDocument(value), [value])
  const [tab, setTab] = useState('structured')
  const validation = useMemo(() => validateUserTaskDocument(document), [document])
  ...
  return h(
    ...
    tab === 'structured'
      ? h(
          'div',
          { style: { display: 'grid', gap: 14 } },
          h(UserTaskDefinitionJsonFormsView, { value: document, onChange: commit, readOnly }),
        )
      : null,

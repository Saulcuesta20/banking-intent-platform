...
import definitionSchema from './process-editor.schema.json'
import definitionUiSchema from './process-editor.ui-schema.json'
import {
...
function ProcessDefinitionJsonFormsView({ value, onChange, readOnly = false }) {
  const payload = clone(value?.payload)
  const definition = useMemo(() => normalizeProcessDefinition(payload), [payload])
  return h(
    EditorFrame,
    {
...
function HistoryPanel({ value, validation }) {
  ...
  return h(
    EditorFrame,
    {
      ...
      subtitle: 'Process structure overview.',
    },
    h('div', { style: { color: colors.muted, fontSize: 12, lineHeight: 1.6 } },
      `Process ID: ${payload.process_id || '—'}. Nodes: ${(payload.nodes || []).length}. Edges: ${(payload.edges || []).length}. Validation: ${validation.valid ? 'ok' : 'review needed'}.`,
    ),
...
export function ProcessEditorView({ value, onChange, readOnly = false }) {
  const document = useMemo(() => normalizeDocument(value), [value])
  const [tab, setTab] = useState('structured')
  const validation = useMemo(() => validateProcessDocument(document), [document])
  ...
  return h(
    ...
    tab === 'structured'
      ? h(
          'div',
          { style: { display: 'grid', gap: 14 } },
          h(ProcessDefinitionJsonFormsView, { value: document, onChange: commit, readOnly }),
        )
      : null,

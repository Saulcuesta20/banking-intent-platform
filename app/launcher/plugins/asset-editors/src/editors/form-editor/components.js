...
import definitionSchema from './form-editor.schema.json'
import definitionUiSchema from './form-editor.ui-schema.json'
import {
...
function FormDefinitionJsonFormsView({ value, onChange, readOnly = false }) {
  const payload = clone(value?.payload)
  const definition = useMemo(() => normalizeFormDefinition(payload), [payload])
  return h(
    EditorFrame,
    {
...
function RelationsPanel({ value }) {
  ...
  return h(
    ...
    {
      title: 'Relations',
      subtitle: 'Associated module and field links.',
    },
    relations.length === 0
    ...
    h('div', { style: { marginTop: 12, fontSize: 12, color: colors.muted } },
      `Module: ${payload.module_id || '—'}  ·  Fields: ${Array.isArray(payload.fields) ? payload.fields.length : 0}`,
    ),
  )
}
...
function HistoryPanel({ value, validation }) {
  ...
  return h(
    EditorFrame,
    {
      ...
      subtitle: 'Form structure overview.',
    },
    h('div', { style: { color: colors.muted, fontSize: 12, lineHeight: 1.6 } },
      `Form ID: ${payload.form_id || '—'}. Module: ${payload.module_id || '—'}. Validation: ${validation.valid ? 'ok' : 'review needed'}.`,
    ),
...
export function FormEditorView({ value, onChange, readOnly = false }) {
  const document = useMemo(() => normalizeDocument(value), [value])
  const [tab, setTab] = useState('structured')
  const validation = useMemo(() => validateFormDocument(document), [document])
  ...
  return h(
    ...
    tab === 'structured'
      ? h(
          'div',
          { style: { display: 'grid', gap: 14 } },
          h(FormDefinitionJsonFormsView, { value: document, onChange: commit, readOnly }),
        )
      : null,

...
import definitionSchema from './business-rule-editor.schema.json'
import definitionUiSchema from './business-rule-editor.ui-schema.json'
import {
...
function RuleDefinitionJsonFormsView({ value, onChange, readOnly = false }) {
  const payload = clone(value?.payload)
  const definition = useMemo(() => normalizeRuleDefinition(payload), [payload])
  return h(
    EditorFrame,
    {
...
function HistoryPanel({ value, validation }) {
  ...
  return h(
    EditorFrame,
    {
      ...
      subtitle: 'Rule structure overview.',
    },
    h('div', { style: { color: colors.muted, fontSize: 12, lineHeight: 1.6 } }, `Conditions: ${conditions.length}. Actions: ${actions.length}. Validation: ${validation.valid ? 'ok' : 'review needed'}.`),
...
export function BusinessRuleEditorView({ value, onChange, readOnly = false }) {
  const document = useMemo(() => normalizeDocument(value), [value])
  const [tab, setTab] = useState('structured')
  const validation = useMemo(() => validateRuleDocument(document), [document])
  ...
  return h(
    ...
    tab === 'structured'
      ? h(
          'div',
          { style: { display: 'grid', gap: 14 } },
          h(RuleDefinitionJsonFormsView, { value: document, onChange: commit, readOnly }),
        )
      : null,

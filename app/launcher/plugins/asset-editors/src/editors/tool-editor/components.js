...
import definitionSchema from './tool-editor.schema.json'
import definitionUiSchema from './tool-editor.ui-schema.json'
import {
...
function ToolDefinitionJsonFormsView({ value, onChange, readOnly = false }) {
  const payload = clone(value?.payload)
  const definition = useMemo(() => normalizeToolDefinition(payload), [payload])
  return h(
    EditorFrame,
    {
...
function HistoryPanel({ value, validation }) {
  ...
  return h(
    EditorFrame,
    {
      ...
      subtitle: 'Tool structure overview.',
    },
    h('div', { style: { color: colors.muted, fontSize: 12, lineHeight: 1.6 } },
      `Tool ID: ${payload.tool_id || '—'}. Type: ${payload.tool_type || '—'}. Operation: ${payload.operation || '—'}. Validation: ${validation.valid ? 'ok' : 'review needed'}.`,
    ),
...
export function ToolEditorView({ value, onChange, readOnly = false }) {
  const document = useMemo(() => normalizeDocument(value), [value])
  const [tab, setTab] = useState('structured')
  const validation = useMemo(() => validateToolDocument(document), [document])
  ...
  return h(
    ...
    tab === 'structured'
      ? h(
          'div',
          { style: { display: 'grid', gap: 14 } },
          h(ToolDefinitionJsonFormsView, { value: document, onChange: commit, readOnly }),
        )
      : null,

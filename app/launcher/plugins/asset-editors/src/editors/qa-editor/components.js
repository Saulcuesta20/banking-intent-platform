...
import definitionSchema from './qa-editor.schema.json'
import definitionUiSchema from './qa-editor.ui-schema.json'
import {
...
function QaDefinitionJsonFormsView({ value, onChange, readOnly = false }) {
  const payload = clone(value?.payload)
  const definition = useMemo(() => normalizeQaDefinition(payload), [payload])
  return h(
    EditorFrame,
    {
      title: 'QA Definition',
...
function HistoryPanel({ value, validation }) {
  ...
  return h(
    EditorFrame,
    {
      ...
      subtitle: 'QA structure overview.',
    },
    h('div', { style: { color: colors.muted, fontSize: 12, lineHeight: 1.6 } },
      `Intent: ${payload.intent || '—'}. Source: ${payload.source || '—'}. Citations: ${(payload.citations || []).length}. Validation: ${validation.valid ? 'ok' : 'review needed'}.`,
    ),
...
export function QaEditorView({ value, onChange, readOnly = false }) {
  const document = useMemo(() => normalizeDocument(value), [value])
  const [tab, setTab] = useState('structured')
  const validation = useMemo(() => validateQaDocument(document), [document])
  ...
  return h(
    ...
    tab === 'structured'
      ? h(
          'div',
          { style: { display: 'grid', gap: 14 } },
          h(QaDefinitionJsonFormsView, { value: document, onChange: commit, readOnly }),
        )
      : null,
